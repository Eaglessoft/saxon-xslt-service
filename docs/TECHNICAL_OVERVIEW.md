...
- `src/main/resources/static/embed`
  - Embeddable widget assets and sample page.
- `src/main/resources`
...
- `XsltTransformationProperties`
  - Binds timeout and size limits from configuration.

## Runtime Flow (High Level)

1. Application starts in Spring Boot embedded Tomcat.
2. Static UI is served from `/` and embed assets are served from `/embed/*`.
3. Client submits XML and XSLT to `POST /transform`.
4. Bean validation and request size validation run before execution.
5. Saxon compiles the stylesheet and performs the transformation.
6. Timeout, output-size, and external-resource restrictions are enforced.
7. JSON success or error response is returned to the client.

## Container Runtime Notes

- Container runs the packaged application from `/app/app.jar`.
- Default internal HTTP port is `8080`.
